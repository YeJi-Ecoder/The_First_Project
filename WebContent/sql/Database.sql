...
select * from 
(select rownum as rnum, boardTitle, boardPrice, boardEa, boardMemo, boardSellerLink, boardTag from 
(select * from userMyTableBoard order by boardID desc)) where boardUserID = ? and boardAvailable = 1;
